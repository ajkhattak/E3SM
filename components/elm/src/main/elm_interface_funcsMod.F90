...
!--------------------------------------------------------------------------------------
  ...
  ! !ARGUMENTS:
    ...
    ! the following currently at column-level, but may be re-defined as grid-level if available (2019-10-28, fmy)
    type(column_carbon_flux)            , intent(in) :: cf_vars
    type(column_nitrogen_flux)          , intent(in) :: nf_vars
    type(column_phosphorus_flux)        , intent(in) :: pf_vars
...
!--------------------------------------------------------------------------------------
  ...
  subroutine elm_bgc_get_data(elm_interface_data,       &
    ...
    associate(&
        decomp_cpools_vr        => cs_vars%decomp_cpools_vr        , & ! Input:  [real(r8) (:,:,:) ]  (gC/m3)  vertically-resolved decomposing (litter, cwd, soil) c pools
        decomp_npools_vr        => ns_vars%decomp_npools_vr        , & ! Input:  [real(r8) (:,:,:) ]  (gC/m3)  vertically-resolved decomposing (litter, cwd, soil) N pools
        decomp_ppools_vr        => ps_vars%decomp_ppools_vr        , & ! Input:  [real(r8) (:,:,:) ]  (gC/m3)  vertically-resolved decomposing (litter, cwd, soil) P pools
        !
        smin_no3_vr             => ns_vars%smin_no3_vr             , & ! [real(r8) (:,:) (gN/m3) vertically-resolved soil mineral NO3
        smin_nh4_vr             => ns_vars%smin_nh4_vr             , & ! [real(r8) (:,:) (gN/m3) vertically-resolved soil mineral NH4
        smin_nh4sorb_vr         => ns_vars%smin_nh4sorb_vr         , & ! [real(r8) (:,:) (gN/m3) vertically-resolved soil mineral NH4 absorbed
        !
        solutionp_vr            => ps_vars%solutionp_vr            , & ! [real(r8) (:,:) (gP/m3) vertically-resolved soil solution P
        labilep_vr              => ps_vars%labilep_vr              , & ! [real(r8) (:,:) (gP/m3) vertically-resolved soil labile mineral P
        secondp_vr              => ps_vars%secondp_vr              , & ! [real(r8) (:,:) (gP/m3) vertically-resolved soil secondary mineralP
        sminp_vr                => ps_vars%sminp_vr                , & ! [real(r8) (:,:) (gP/m3) vertically-resolved soil mineral P = solutionp + labilep + secondp
        occlp_vr                => ps_vars%occlp_vr                , & ! [real(r8) (:,:) (gP/m3) vertically-resolved soil occluded mineral P
        primp_vr                => ps_vars%primp_vr                , & ! [real(r8) (:,:) (gP/m3) vertically-resolved soil primary mineral P
        !
        plant_ndemand           => nf_vars%plant_ndemand           , &
        plant_pdemand           => pf_vars%plant_pdemand           , &
        !
        ...
        !
        soilpsi                 => ws_vars%soilp                            , & ! Input:  [real(r8) (:,:)  ]  soil water potential in each soil layer (Pa)
        h2osoi_vol              => ws_vars%h2osoi_vol                       , & ! Input:  [real(r8) (:,:)  ]  volumetric soil water (0<=h2osoi_vol<=watsat) [m3/m3]  (nlevgrnd)
        h2osoi_liq              => ws_vars%h2osoi_liq                       , & ! Input:  [real(r8) (:,:)  ]  liquid water (kg/m2) (new) (-nlevsno+1:nlevgrnd)
        !
        t_soisno                => es_vars%t_soisno                         , & ! Input:  [real(r8) (:,:)  ]  soil temperature (Kelvin)  (-nlevsno+1:nlevgrnd)
        !
        o2_decomp_depth_unsat   => ch4_vars%o2_decomp_depth_unsat_col       , & ! Input:  [real(r8) (:,:)  ]  O2 consumption during decomposition in each soil layer (nlevsoi) (mol/m3/s)
        o2_decomp_depth_sat     => ch4_vars%o2_decomp_depth_sat_col         , & ! Input:  [real(r8) (:,:)  ]  O2 consumption during decomposition in each soil layer (nlevsoi) (mol/m3/s)
        conc_o2_unsat           => ch4_vars%conc_o2_unsat_col               , & ! Input:  [real(r8) (:,:)  ]  O2 conc in each soil layer (mol/m3) (nlevsoi)
...
!--------------------------------------------------------------------------------------
  ! !INTERFACE:
  ! pass data from elm_bgc to elm_bgc_data
  subroutine elm_bgc_update_data(elm_bgc_data, bounds,  &
    ...
    associate(                                                                             &
         ...
         actual_immob_no3_vr          => nf_vars%actual_immob_no3_vr                      , & ! Output: [real(r8) (:,:) ]
         actual_immob_nh4_vr          => nf_vars%actual_immob_nh4_vr                      , & ! Output: [real(r8) (:,:) ]
         smin_no3_to_plant_vr         => nf_vars%smin_no3_to_plant_vr                     , & ! Output: [real(r8) (:,:) ]
         smin_nh4_to_plant_vr         => nf_vars%smin_nh4_to_plant_vr                     , & ! Output: [real(r8) (:,:) ]
         n2_n2o_ratio_denit_vr        => nf_vars%n2_n2o_ratio_denit_vr                    , & ! Output: [real(r8) (:,:) ]  ratio of N2 to N2O production by denitrification [gN/gN]
         f_n2o_denit_vr               => nf_vars%f_n2o_denit_vr                           , & ! Output: [real(r8) (:,:) ]  flux of N2O from denitrification [gN/m3/s]
         f_n2o_nit_vr                 => nf_vars%f_n2o_nit_vr                             , & ! Output: [real(r8) (:,:) ]  flux of N2O from nitrification [gN/m3/s]
         supplement_to_sminn_vr       => nf_vars%supplement_to_sminn_vr                   , & ! Output: [real(r8) (:,:) ]
         sminn_to_plant_vr            => nf_vars%sminn_to_plant_vr                        , & ! Output: [real(r8) (:,:) ]
         actual_immob_vr              => nf_vars%actual_immob_vr                          , & ! Output: [real(r8) (:,:) ]
         !
         potential_immob_p            => pf_vars%potential_immob_p                        , & ! Output: [real(r8) (:)   ]
         actual_immob_p               => pf_vars%actual_immob_p                           , & ! Output: [real(r8) (:)   ]
         sminp_to_plant               => pf_vars%sminp_to_plant                           , & ! Output: [real(r8) (:)   ]
         supplement_to_sminp_vr       => pf_vars%supplement_to_sminp_vr                   , & ! Output: [real(r8) (:,:) ]
         sminp_to_plant_vr            => pf_vars%sminp_to_plant_vr                        , & ! Output: [real(r8) (:,:) ]
         actual_immob_p_vr            => pf_vars%actual_immob_p_vr                        , & ! Output: [real(r8) (:,:) ]
         !
         decomp_cascade_ntransfer_vr      => nf_vars%decomp_cascade_ntransfer_vr       , & ! Output: [real(r8) (:,:,:) ]  vert-res transfer of N from donor to receiver pool along decomp. cascade (gN/m3/s)
         decomp_cascade_sminn_flux_vr     => nf_vars%decomp_cascade_sminn_flux_vr      , & ! Output: [real(r8) (:,:,:) ]  vert-res mineral N flux for transition along decomposition cascade (gN/m3/s)
         potential_immob_vr               => nf_vars%potential_immob_vr                , & ! Output: [real(r8) (:,:)   ]
         sminn_to_denit_decomp_cascade_vr => nf_vars%sminn_to_denit_decomp_cascade_vr  , & ! Output: [real(r8) (:,:,:) ]
         gross_nmin_vr                    => nf_vars%gross_nmin_vr                     , & ! Output: [real(r8) (:,:)   ]
         net_nmin_vr                      => nf_vars%net_nmin_vr                       , & ! Output: [real(r8) (:,:)   ]
         gross_nmin                       => nf_vars%gross_nmin                        , & ! Output: [real(r8) (:)     ]  gross rate of N mineralization (gN/m2/s)
         net_nmin                         => nf_vars%net_nmin                          , & ! Output: [real(r8) (:)     ]  net rate of N mineralization (gN/m2/s)
         ! add phosphorus
         decomp_cascade_ptransfer_vr      => pf_vars%decomp_cascade_ptransfer_vr       , & ! Output: [real(r8) (:,:,:) ]  vert-res transfer of P from donor to receiver pool along decomp. cascade (gP/m3/s)
         decomp_cascade_sminp_flux_vr     => pf_vars%decomp_cascade_sminp_flux_vr      , & ! Output: [real(r8) (:,:,:) ]  vert-res mineral P flux for transition along decomposition cascade (gP/m3/s)
         potential_immob_p_vr             => pf_vars%potential_immob_p_vr              , & ! Output: [real(r8) (:,:)   ]
         gross_pmin_vr                    => pf_vars%gross_pmin_vr                     , & ! Output: [real(r8) (:,:)   ]
         net_pmin_vr                      => pf_vars%net_pmin_vr                       , & ! Output: [real(r8) (:,:)   ]
         gross_pmin                       => pf_vars%gross_pmin                        , & ! Output: [real(r8) (:)     ]  gross rate of P mineralization (gP/m2/s)
         net_pmin                         => pf_vars%net_pmin                          , & ! Output: [real(r8) (:)     ]  net rate of P mineralization (gP/m2/s)
         !
         decomp_cascade_hr_vr             => cf_vars%decomp_cascade_hr_vr             , & ! Output: [real(r8) (:,:,:) ]  vertically-resolved het. resp. from decomposing C pools (gC/m3/s)
         decomp_cascade_ctransfer_vr      => cf_vars%decomp_cascade_ctransfer_vr      , & ! Output: [real(r8) (:,:,:) ]  vertically-resolved het. resp. from decomposing C pools (gC/m3/s)
         phr_vr                           => cf_vars%phr_vr                           , & ! Output: [real(r8) (:,:)   ]  potential HR (gC/m3/s)
         fphr                             => cf_vars%fphr                               & ! Output: [real(r8) (:,:)   ]  fraction of potential SOM + LITTER heterotrophic
         )
...
!--------------------------------------------------------------------------------------
! END of elm-bgc through interface
!--------------------------------------------------------------------------------------
